...
CREATE TABLE tasks (
  ...
  status TEXT DEFAULT 'pending',    -- 状态: pending/in-progress/completed
  priority INTEGER DEFAULT 1,       -- 优先级 (1-10)
  -- 扩展字段 (v2.0)
  process_order_id TEXT,            -- 委托加工单ID
  factory_code TEXT,                -- 工厂编号
  order_date TEXT,                  -- 委托日期
  delivery_time TEXT,               -- 送达时间
  quantity INTEGER,                 -- 数量
  assigned_person TEXT,             -- 委托人
  assigned_team TEXT,               -- 委托班组
...
#### 版本历史
- **版本1**: 初始数据库结构，基础任务和师傅管理
- **版本2**: 添加扩展字段支持，包含更多导入数据字段

#### 自动版本检测机制
```typescript
// 版本检测和升级逻辑
const savedVersion = localStorage.getItem('sqlite_db_version')
if (savedVersion && parseInt(savedVersion) !== this.config.version) {
...
#### 版本升级策略
1. **自动检测**: 应用启动时自动检测数据库版本
2. **结构重建**: 版本不匹配时重新创建数据库表结构  
3. **数据保护**: 升级前自动备份现有数据
4. **向后兼容**: 支持从旧版本数据恢复
...
// 错误示例 - 这会导致数据无法显示
task.status = 'inProgress'  // ❌ 错误

// 正确示例 
task.status = 'in-progress'  // ✅ 正确
```
...
#### 3. 数据库表结构不匹配
**错误信息**: `table tasks has no column named process_order_id`

**解决方案**:
1. 刷新浏览器清除缓存
2. 系统会自动检测版本并重建数据库
3. 或手动清除localStorage中的数据库缓存

#### 4. 导入数据失败
- 检查文件格式 (支持Excel/.xlsx和CSV)
- 验证数据完整性和列映射
- 查看控制台错误信息
- 确认师傅名称在预定义列表中
